...
export default function ProductDetail() {
    ...
    const [showReturnModal, setShowReturnModal] = useState(false); // 🔄 return confirmation modal
        ...
        const fetchUniqueCode = async () => {
            try {
                ...
                if (deliveredOrder) {
                    ...
                    setIsReturnEligible(true);
                    setOrderId(deliveredOrder._id);
                }
            } catch (err) {
    ...
    const returnProductHandler = async () => {
        try {
            if (!orderId) throw new Error("Order ID not found");

            const { data } = await axios.put(`/api/v1/order/return/${orderId}`);
            toast(data.message, {
                type: 'success',
                position: toast.POSITION.BOTTOM_CENTER
            });
            setIsReturnEligible(false);
            setShowReturnModal(false);
        } catch (error) {
            toast(error.response?.data?.message || error.message || 'Return failed', {
                type: 'error',
                position: toast.POSITION.BOTTOM_CENTER
            });
            setShowReturnModal(false);
                ...
                <Fragment>
                    ...
                    <Modal show={showReturnModal} onHide={() => setShowReturnModal(false)} centered>
                        ...
                        <Modal.Footer>
                            ...
                            <button className="btn btn-danger" onClick={returnProductHandler}>
                                Yes, Return
                            </button>
                        </Modal.Footer>
                    </Modal>
